...
ar1_matrix <- function(n, rho, sigma = 1, inverse = FALSE){
	if( inverse ){
		...
		(
			Matrix::Diagonal(n = n)*(1 - rho^2) - (t(L) + L)*rho + ( crossprod(L) + tcrossprod(L) )*(rho^2)
		)/sigma
	}else{
...
# ----------------------------------------------------------------
#  Functions for lag distribution 
# ----------------------------------------------------------------

# Construct discretized time-lag PMF using truncated negative binomial distribution
get_NB_lag_probs <- function(val, nn, mu){
	pp <- dnbinom(0:(nn-1), mu = mu, size = val)
	pp/sum(pp)
}

# Experimental/not used: Optimize truncated NB parameters for observed lags
optim_NB_lag_probs <- function(count_matrix, mu_par, return_par = FALSE, lower = 0.05, upper = 500){
	
	nk <- ncol(count_matrix)
	count_matrix <- t(count_matrix)
	
	obj_fun <- function(x){
		sum(log(get_NB_lag_probs(val=x, nn = nk, mu = mu_par)) * count_matrix)
	}
	
	opt_par <- optim(par = 1, fn = obj_fun, control = list(fnscale = -1), method = "Brent", lower = lower, upper = upper )$par
	...
	if(return_par){
		...
		list(
			opt_par = opt_par, 
			probs = get_NB_lag_probs(val = opt_par, nn = nk, mu = mu_par)
		)
	}
}
...
fitMERMAID <- function(rt_formula, pi_formula, pi_intercept, data, prev_data = NULL, prev_weight = 1, mean_lag = 7, max_lag = 15, confirmed_cases_var = "new_cases", date_var = "date", subset_var = NULL, region_var = NULL, pop_size_var = NULL, si_nts = 60, si_weights, lag_probs = NULL, adj_Lambda = 1, init_Lambda = 0, min_pi_t = 0.01, glm_method = 2, tol = 1e-3, recycle_start = TRUE, plot_every = NA, state_name = "", init_Rt = NA, recovered_offset = TRUE, return_vcov = FALSE, max_it = 5000, max_it_E_step = 5, pi_start = NULL, pi_init_mean = 0.5, min_p_obs = 0.8, accelerated_em = TRUE, pi_eps = 0.05, reweight_prev = FALSE, prev_method = "gaussian", print_mem = TRUE, info_prev_weight = 1, crude_pi_se_adjustment = TRUE ){
	...
	if( is.null(lag_probs ) ){
		lag_probs <- get_NB_lag_probs(val = 2, nn = nk, mu = mean_lag)
	}else{
		fix_lag_probs <- TRUE
	}
	...
	calc_information <- function(prev_w = prev_weight){
		...
		if( estimate_weekday_lag ){
			...
			stop("Estimating weekday lags is an unsupported experimental feature.")
		...
		inner <- as.matrix(J_hes + THiTt)
	
	
		list(
			grad_vec = grad_vec, 
			adj_mat = THiTt, 
			info_louis = J_hes + THiTt, 
			info_hess = J_hes, 
			VarY_C = (-1)*JHiJt 
		)
	}
	...
	repeat{
		...
		if( !fix_lag_probs ){
			...
			stop("ERROR: Estimating lag distribution is an experimental feature; not currently supported.")
		
			opt_lag_probs <- optim_NB_lag_probs(A_start, mean_lag)
		...
		if( !is.null(theta_last) & accelerated_em & it > 1 ){
			...
			theta_adj <- solve(
				xinfo$info_hess + acc_nudge*xinfo$adj_mat, 
				xinfo$grad_vec
			)[,1]
		...
		if( abs(ll_delta) + max(abs(xinfo$grad_vec)/sqrt(abs(diag(xinfo$info_hess))+0.5)) < tol ){  #& ll_delta > 0 ){
			if( final_pass ){
				break;
			}else{
				final_pass <- TRUE
	...
	xinfo <- calc_information(prev_w = prev_weight)

	J_mat <- xinfo$info_louis
	J_hes <- xinfo$info_hess

	# J_w <- xinfo_w$info_louis
	# J_1 <- xinfo_1$info_louis	


	if( any(diag( J_mat ) < 0 ) ){
		wneg <- which(diag( J_mat ) < 0)
		warning(paste0(length(wneg), " negative variances in information matrix."))
		J_mat[wneg,] <- J_hes[wneg,]
		J_mat[,wneg] <- J_hes[,wneg]
	}
	...
	if( crude_pi_se_adjustment ){
		logitPi_var_diag <- prev_weight * logitPi_var_diag
	}
	...
	if(return_vcov){
		...
		out_list$info_louis_mat <- xinfo$info_louis
		out_list$info_hess_mat <- xinfo$info_hess 
