...
P_sero <- 0.01 
out_dir <- "simulation_output_custom_NSS"
...
time_int <- seq(ndays)
X_rt <- splines::bs( time_int, df = rt_spline_df, degree = 3)
...
rt_beta_vec_list <- list(
	c( 0.633, -0.441, -0.45, -0.565, 0.55, -0.487, -0.89, -0.464 ),
	c( 0.679, -0.423, -0.423, -0.757, 0.634, -0.609, -0.832, -0.201 ),
	c( 0.625, -0.507, -0.389, -0.667, 0.559, -0.346, -0.869, -0.478 )
)
...
while( n_sim < num_regions ){
	...
	sim$data$pop_size <- pop_size # ceiling( sum(sim$data$Y)/final_prev )
...
if( num_regions == 1 ){
	...
	rt_eqn <- ~ bs(max(date) - date, degree = 3, df = rt_spline_df )
	pi_eqn <- ~ ntest

}else{

	rt_eqn <- ~ Intervention + factor(region)*bs(max(date) - date, degree = 3, df = rt_spline_df )
