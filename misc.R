...
tmp <- transmute(covid, date = date, confirm = doh_n_tuvong)
library(ggplot2)
ggplot(data = subset(tmp, date >= "2021-10-01"), aes(x = date, y = confirm)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = confirm, y = confirm + 5), vjust = 0) +
  theme_bw() +
  ylab("Số ca tử vong") +
  scale_x_date(name = "Ngày", breaks = "1 week")

with(subset(tmp, date >= "2021-10-01"), sum(confirm))
...
ggplot(data = subset(tmp, date >= "2021-10-01"), aes(x = date, y = confirm)) +
  ...
  theme_bw() +
  ylab("Số ca tử vong") +
  scale_x_date(name = "Ngày", breaks = "1 week", date_labels = "%d-%m-%Y") +
  theme(legend.position = "none")


with(subset(tmp, date >= "2021-10-01" & month == 10), sum(confirm)) ## 1831
with(subset(tmp, date >= "2021-10-01" & month == 11), sum(confirm)) ## 582
